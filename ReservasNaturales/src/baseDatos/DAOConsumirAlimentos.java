...
public class DAOConsumirAlimentos extends AbstractDAO {
    ...
    public DAOConsumirAlimentos(Connection conexion, FachadaAplicacion fa) {
        ...
        obtenerConsumirAlimentos();
    }

    public List<ConsumirAlimento> obtenerConsumirAlimentos() {
    ...
    int borrarConsumirAlimento(int idEspecie, String nombreEspecie) {
        Connection con;
        PreparedStatement stmConsAlimento=null;
        int res = -1;
        con=super.getConexion();
        ...
        try {
        stmConsAlimento=con.prepareStatement("delete from consumirAlimentos where id_especie = ? and nombre_especie = ?");
